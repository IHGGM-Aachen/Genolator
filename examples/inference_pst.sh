#!/usr/bin/env bash
# Generation against the published DNA + PST checkpoint.
#
# CHECKPOINT_DIR must contain the three artefacts written by training:
#   genolator_dna_and_pst.pt, dna_projector.pt, pst_projector.pt
#
# The published PST run evaluated the full test set, so --kinds is omitted.
#
...
CHECKPOINT_DIR="${CHECKPOINT_DIR:?set CHECKPOINT_DIR to the genolator_v1_dna_and_pst model folder}"
OUTPUT_DIR="${OUTPUT_DIR:-./inference/genolator_pst_v1}"
...
python3 ../run_inference.py \
    ...
    --output_suffix "test" \
    --trained_model_path "$CHECKPOINT_DIR/genolator_dna_and_pst.pt" \
    --trained_dna_projector_path "$CHECKPOINT_DIR/dna_projector.pt" \
    --trained_protein_projector_path "$CHECKPOINT_DIR/pst_projector.pt" \
    --hf_token "$HF_TOKEN" \
